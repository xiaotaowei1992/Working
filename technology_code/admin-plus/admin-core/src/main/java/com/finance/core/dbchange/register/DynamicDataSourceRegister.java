package com.finance.core.dbchange.register;

import com.finance.core.dbchange.config.DynamicDataSourceContextHolder;
import com.finance.core.dbchange.config.DynamicRoutingDataSource;
...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        defauleDataSourceProperties = binder.bind("spring.datasource.fmis", Map.class).get();
        // 获取数据源类型
        String typeStr = evn.getProperty("spring.datasource.fmis.type");
        // 获取数据源类型
        Class<? extends DataSource> clazz = getDataSourceType(typeStr);
        // 绑定默认数据源参数 也就是主数据源
        DataSource consumerDatasource, defaultDatasource = bind(clazz, defauleDataSourceProperties);
        DynamicDataSourceContextHolder.dataSourceIds.add("fmis");
        log.info("注册默认数据源成功");
        // 获取其他数据源配置
